...
export default function Setor({ session }) {
  ...
  const [sector_id, setSetorId] = useState("");

  const onSubmit = (data) => {
    api
      .post("/insertCentroDeCusto", {
        ...data,
  ...
  function editar({sector_id, sector_desc}){
      console.log(sector_desc)
    setSetorId(sector_id)
    showModalEditar(true)

    setTimeout(function(){ 
        setValue("nome", sector_desc);
    }, 50);
...
Setor.getInitialProps = async (ctx) => {
  const { req, res } = ctx;
